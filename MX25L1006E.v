...
// *============================================================================================== 
    ...
    `define	 CMD_STATE			1
    `define	 BAD_CMD_STATE		2		// invalid command state
...
// *============================================================================================== 
    ...
    inout  WP;		// Write Protect (Low active)
    inout  HOLD; 	// HOLD (Low active) (suspend command)
...
// *============================================================================================== 
    ...
    /*----------------------------------------------------------------------*/
    parameter	A_MSB			= 16,			// Address MSB (17 bits)
				TOP_Add			= 17'h1ffff,	// Top memory address (1ffff = 131071 -> 131072 bytes = 1M-bit)
                A_MSB_SFDP      = 6,			// Address MSB for SFDP (7 bits)
                SFDP_TOP_Add    = 7'h7f,		// Top memory address for SFDP (7f = 127 -> 128 bytes)
				Sector_MSB		= 4,			// Sector MSB (5 bits)
				Block_MSB		= 1,			// Block MSB (2 bits)
				Block_NUM		= 2;			// Number of Blocks

    /*----------------------------------------------------------------------*/
    /* Define ID Parameter						    						*/
    /*----------------------------------------------------------------------*/
    parameter	ID_MXIC			= 8'hc2,		// Manufacturer ID
				ID_Device		= 8'h10,		// Device ID
				Memory_Type		= 8'h20,		// Memory Type
				Memory_Density	= 8'h11;		// Memory Capacity (11h = 17bit address -> 1M-bit)

    /*----------------------------------------------------------------------*/
    /* Define Initial Memory File Name					    				*/
    /*----------------------------------------------------------------------*/
    //parameter   Init_File	= "none"; // initial flash data
	parameter   Init_File		= "../../design/flash_data.txt";	// initial flash data for testing
    parameter   Init_File_SFDP	= `File_Name_SFDP;  				// initial flash data for SFDP

    /*----------------------------------------------------------------------*/
    /* AC Characters Parameter						    					*/
    /*----------------------------------------------------------------------*/
    parameter	tSHQZ	= 6,    		// CS High to SO Float Time [ns] (High-Z)
				tCLQV	= `Vtclqv,    	// Clock Low to Output Valid
				tCLQX	= 0,   			// Output Hold Time
                tHHQX   = 6,    		// HOLD High to Output Low-z
                tHLQZ   = 6,    		// HOLD Low to Output High-z
             	tBP  	= 9_000,		// Byte program time (write time)
             	tSE		= 40_000_000,	// Sector erase time  
				tBE		= 400_000_000,	// Block erase time
				tCE		= 800_000_000,	// chip erase time
				tPP		= 600_000,		// Program time
				tW 		= 5_000_000,	// Write Status time (flash status register)
	...
	specparam   tSCLK   = 9.6,		// Clock Cycle Time [ns]
                ...
                tHLCH   = 5,        // HOLD# Low Setup Time (relative to SCLK) (min) [ns]
                tCHHH   = 5,        // HOLD# High Hold  Time (relative to SCLK) (min) [ns]
                tHHCH   = 5,        // HOLD High Setup Time (relative to SCLK) (min) [ns]
                tCHHL   = 5,        // HOLD Low Hold  Time (relative to SCLK) (min) [ns]
    ...
    reg	    SI_OUT_EN;   	
    reg	    SO_OUT_EN;   
    reg     HOLD_OUT_B;			// Truely HOLD (low active)
    wire    HOLD_B_INT;			// Internal HOLD signal (low active)
    reg     DP_Mode;	    
    reg     Read_1XIO_Mode;
    reg     Read_1XIO_Chk;
    ...
    reg     Read_2XIO_Mode;
    reg     Read_2XIO_Chk;
    reg     Byte_PGM_Mode;		// Byte Program Mode
    reg     Read_SHSL;			
    reg     tDP_Chk;
    reg     tRES1_Chk;
    reg     tRES2_Chk;
    wire    Write_SHSL;
    wire    WP_B_INT;
    wire    ISCLK;				// Internal SCLK
    wire    WIP;				// Write In Progress Bit
    wire    WEL;				// Write Enable Latch Bit
    wire    SRWD;				// Status Register Write Disable Bit	
    wire    Dis_CE;				
    wire    Dis_WRSR;			// disable write status register 
    event   WRSR_Event; 
    event   BE_Event;
    event   SE_4K_Event;
    event   CE_Event;
    ...
    integer i;
    integer j;
    integer Bit; 				// bit counter
    integer Bit_Tmp; 			// temporary bit counter
    integer Start_Add;
    integer End_Add;
    integer Page_Size;
    ...
    begin : memory_initialize
        ...
        // 如果有指定外部 txt 檔案，就用檔案內容覆蓋上面的 NOP
        if ( Init_File != "none" )
...
//
//		ARRAY[4096] = 8'h04;		// test for 4KB sector erase
//	
//		if ( Init_File != "none" )
//	    	$readmemh(Init_File,ARRAY);
        ...
        // define SFDP code
        SFDP_ARRAY[8'h0] =  8'h53;		// 'S'
        SFDP_ARRAY[8'h1] =  8'h46;		// 'F'
        SFDP_ARRAY[8'h2] =  8'h44;		// 'D'
        SFDP_ARRAY[8'h3] =  8'h50;		// 'P'
...
// *============================================================================================== 
    ...
    always @ ( HOLD_B_INT or negedge SCLK ) begin
        if ( HOLD_B_INT == 1'b0 && SCLK == 1'b0) begin			// Hold Active
            SCLK_EN = 1'b0;
            HOLD_OUT_B <= #tHLQZ 1'b0;
        end
        else if ( HOLD_B_INT == 1'b1 && SCLK == 1'b0 ) begin		// Hold Inactive
            SCLK_EN = 1'b1;
...
// *============================================================================================== 
    ...
    assign Dis_WRSR = WP_B_INT == 1'b0 && SRWD == 1'b1;
	...
	if ( CS == 1'b0 ) begin						
	    Bit_Tmp = Bit_Tmp + 1; 						// blocking assignment
	    Bit	= Bit_Tmp - 1;
    ...
    always @ (posedge CS) begin
		...
		SIO0_Reg 			<= #tSHQZ 1'bx;
		SIO1_Reg 			<= #tSHQZ 1'bx;
		#1;					// delay for upper always block
		Bit					= 1'b0;
		Bit_Tmp	    		= 1'b0;
		SI_IN_EN			= 1'b0;
...
// *========================================================================================== 
	...
	begin
	    ...
	    forever @ ( negedge ISCLK or  posedge CS ) begin
	        if ( CS == 1'b1 ) begin
		    	...
		    	SI_IN_EN  = 1'b0;
		    	if ( Dummy_Count ) begin
					{SIO1_Reg, SIO0_Reg, OUT_Buf} <= #tCLQV {OUT_Buf, OUT_Buf[7:6]};
	    	 		Dummy_Count = Dummy_Count - 1;
		    	end
		    	else begin
					Address = Address + 1;
	...
	begin
	    ...
	    Tmp_Int = 0;				// count how many Bits been shifted
        Offset  = Address[7:0];
	    /*------------------------------------------------*/
	...
	begin
	    ...
	    #program_time ;
	    for ( i = 0; i < Dummy_Count; i = i + 1 ) begin
			ARRAY[Address+ i] = ARRAY[Address + i] & Dummy_A[i];	// program can only change bits from 1 to 0
	    end
		// WIP : write in process Bit
	    Status_Reg[0] = 1'b0;
		// WEL : write enable latch
