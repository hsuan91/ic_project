...
Reg_file u_Reg_file(
	.clk(clk),
	.rst(rst),
	.write_regf_en(write_regf_en_rr),
	.addr_rd(addr_rd_rr),
	.addr_rs1(addr_rs1_),
	.addr_rs2(addr_rs2_),
	.rd_value(rd_value_),
...
always_comb begin
	unique case(sel_rd_value_rr)
		0: 	rd_value_ = alu_out_r;
		1: 	rd_value_ = read_data;
		2: 	rd_value_ = mul_out_r;
		3: 	rd_value_ = div_out_r;
	endcase
end
...
always_ff @(posedge clk) begin
	if(rst || flush_EXWB_r) begin
		...
		mul_out_r             <= mul_out;
		div_out_r             <= div_out;
		sel_rd_value_rr       <= sel_rd_value_r;
	end
end
